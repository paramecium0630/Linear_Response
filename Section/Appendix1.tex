...
Consider a network with $N$ nodes whose intrinsic one-dimensional dynamics, $x_i(t)$,  is described by the nonlinear function $f_i(x_i;r_i)$, where $r_i$ denotes the node parameter. Nodes $i$ and $j$ are connected by directed and weighted links given by the matrix ${\bf W}$. The dynamics are governed by
 \begin{eqnarray}
...
$h$ is a coupling function that describes the interaction between nodes $i$ and $j$. Furthermore, we assume that the system approaches some asymptotic dynamics in the presence of noise, and $x_i(t)$  fluctuates around its mean value $\langle x_i\rangle$ (local maximum of the probability density function $\psi_{ss}({\vec x})$  which is close to the noise-free solution $X_i$. Then up to linear order in $\Delta{\vec x}\equiv{\vec x}-{\vec X}  $, one has
 \begin{eqnarray}
