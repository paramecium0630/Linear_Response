...
The transient formulas in Sec. \ref{sec:time-dependent} follow from the same biorthogonal decomposition. Since
\begin{equation}
...
\end{equation}
For the cosine protocol in Eq. (\ref{dra(t)cos}), this is the same quantity introduced in Eq. (\ref{Iml}); its explicit piecewise form is given there. The edge-perturbation formulas in Sec. \ref{sec:time-dependent} follow from the same expansion, with only the prefactor and the covariance derivative replaced according to the perturbation under consideration.
