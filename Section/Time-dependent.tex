...
\quad The steady-state response developed above characterizes the difference between two nearby stationary states before and after a small parameter perturbation. However, a perturbation can be applied through a finite-time protocol rather than as an instantaneous change. We consider parameter protocols that drive the system from one steady state to a new one. Assume a system that initially fluctuates near a fixed point, and the time-dependent response theory is derived to predict the average change of some observables as the function of time. The time-dependent responses of the mean state, deterministic force, and equal-time covariance with respect to the network parameters are derived. The numerical validation is then presented for linear network dynamics in both an equilibrium reference network and a nonequilibrium steady-state network.
\subsection{General Formula for Time-Dependent Linear Response Function}
\quad We consider the time-dependent response induced by a time-dependent perturbation of the network parameters, $\vec{p}\rightarrow\vec{p}+\delta \vec{p}(t)$, and assume that the system is in the unperturbed steady state for $t\le 0$. The time-dependent linear response function of an observable $\mathbf{B}(t)$ is then
defined as
\begin{equation}
...
\end{equation}
The conjugate force $\vec{\mathscr{F}}(t)$ is defined in the same way as in (\ref{conjugate_force}). The time-dependent linear response function is then given by
\begin{eqnarray}
...
\end{cases}.
\end{equation}
The ensemble averages of $\vec{x}(t)$ and $\vec{F}(t)$ do not depend on the noise strengths and are determined solely by ${\bf Q}$. From Eq. (\ref{chitKolinear}), the time-dependent response of ${\bf K}(t,t)$ to the
...
\end{equation}
where $\mathbf{K}_0$ means the steady-state covariance when $t\leq 0$. Using the same spectral decomposition, the covariance integral can also be
...
\end{eqnarray}
Here the protocol integral $I_{m\ell}(t)$ is the same quantity defined in Eq. (\ref{Iml}), since the edge perturbation uses the same cosine time dependence. Figures \ref{Time-dependent_undirected_dW} and \ref{Time-dependent_directed_dW} show the corresponding validation for a time-dependent edge perturbation. The response is driven by the contrast $X_\beta-X_\alpha$ across the perturbed link, so the transient signals directly probe how a local change of coupling propagates through the network and modifies both the deterministic relaxation and the covariance dynamics.\par
