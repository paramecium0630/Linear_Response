...
\quad We now consider parameter protocols that drive the system from one steady state to a new one. We derive a  time-dependent response formula for noisy network dynamics fluctuating a fixed point before perturbation. The time-dependent responses of the mean state, deterministic force, and equal-time covariance with respect to the network parameters are derived explicitly. Explicit closed-form results and numerical validation are then presented for linear network dynamics in both an equilibrium reference network and a nonequilibrium steady-state network.
