...
\subsection{Response Kernels in the Linearized-Dynamics Framework}
...
\end{equation}
For completeness, we record the closed-form expressions for the cosine protocol, although the validation below does not rely on their detailed algebraic form.
...
\begin{equation}
     ...
     \end{cases}.
\end{equation}
As $t\to\infty$, since $\lambda_k<0$, $\langle x_i(t)\rangle$ approaches the new noise-free fixed point $X_i-b (a_\alpha-X_\alpha)Q^{-1}_{i\alpha}$ exponentially, with a time scale determined by the spectrum $\{\lambda_k \}$ of ${\bf Q}$. For the force response in linear network dynamics driven by $\delta r_\alpha(t)$, we use
...
\end{equation}
For the driving protocol $\delta r_\alpha(t)$ over $0\leq t\leq\tau$, with $\delta r_\alpha(0)=0$ and $\delta {\dot r}_\alpha(t>\tau)=0$, the mean-force response becomes
\begin{equation}
...
\end{cases}.
\end{equation}
The ensemble averages of $\vec{x}(t)$ and $\vec{F}(t)$ do not depend on the noise strengths and are determined solely by ${\bf Q}$. From Eq. (\ref{chitKolinear}), the time-dependent response of ${\bf K}(t,t)$ to a change in $r_\alpha(t)$ is
\begin{equation}
...
\end{equation}
where $\mathbf{K}_0$ denotes the steady-state covariance for $t\leq 0$. Using the same spectral decomposition, the covariance integral can also be evaluated explicitly. For the cosine protocol (\ref{dra(t)cos}), we define
\begin{equation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
The derivation is summarized in Appendix C. The evolution of ${\bf K_0}(t)$ induced by the time-dependent change in $r_\alpha$ is
\begin{equation}
...
\begin{eqnarray}
    ...
    \label{xi(t)dWlinear}
\end{eqnarray}
The same spectral decomposition described in Appendix C can be used to evaluate these convolution integrals explicitly. We again use the protocol (\ref{dra(t)cos}) on the connection between node $\alpha$ and node $\beta$:
\begin{equation}
