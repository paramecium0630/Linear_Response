...
\quad The steady-state response developed above characterizes the difference between two nearby stationary states before and after a small parameter perturbation. In many situations, however, a perturbation is applied through a finite-time protocol. We therefore consider parameter protocols that drive the system from one steady state to another. Starting from a system that initially fluctuates at a steady state, we derive a time-dependent response theory that predicts the average change of observables as a function of time. As in Secs.~III and IV, the explicit formulas in this section are exact for the linearized Ornstein-Uhlenbeck dynamics and provide the corresponding local fixed-point approximation for the original nonlinear network. In particular, we derive the responses of the mean state, deterministic force, and equal-time covariance to time-dependent perturbations of the network parameters. We then validate the theory numerically for linear network dynamics in both an equilibrium reference network and a nonequilibrium steady-state network.
...
\begin{equation}
    h(x_i,x_j)=x_j-x_i .
\end{equation}
For the undirected network, all nodes have identical noise strengths, $\sigma_{ii}=0.01$, so the system serves as an equilibrium reference case. In contrast, the directed network realizes a nonequilibrium steady state through its asymmetric couplings.\par
...
\begin{figure}[H]
    \centering
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_xunddr.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_xdirdr.eps}}\\
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_funddr.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_fdirdr.eps}}\\
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_Kounddr.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_Kodirdr.eps}}
    \caption{Comparison between Langevin simulations (dashed) and the time-dependent linear-response prediction (solid) for a node perturbation $\delta r_1(t)$ in the four-node networks of Fig. \ref{graphN4}. The left column corresponds to the undirected equilibrium network [Fig. \ref{graphN4}a], and the right column to the directed nonequilibrium network [Fig. \ref{graphN4}b]. Panels (a) and (b) show the mean-state responses $\langle x_1(t)\rangle-\langle x_1(0)\rangle$ and $\langle x_2(t)\rangle-\langle x_2(0)\rangle$. Panels (c) and (d) show the mean-force responses $\langle F_1(t)\rangle$ and $\langle F_2(t)\rangle$. Panels (e) and (f) show the equal-time covariance responses for $K_{11}(t,t)$ and $K_{12}(t,t)$. The close overlap between theory and simulation over both the driving stage and the post-driving relaxation verifies the time-dependent response theory in both the equilibrium and nonequilibrium cases.}
    \label{Time-dependent_dr}
\end{figure}
Figure \ref{Time-dependent_dr} compares these predictions with Langevin simulations for a perturbation of the node parameter $r_1(t)$. The curves resolve both stages of the response: the driven interval $0\le t\le \tau$, during which the observables follow the smooth protocol, and the subsequent relaxation for $t>\tau$, during which the system approaches the new steady state. Agreement for $\langle x_i(t)\rangle$ and $\langle F_i(t)\rangle$ verifies the predicted mean relaxation dynamics. Agreement for $K_{ij}(t,t)$ shows that the theory also captures the time evolution of fluctuations and correlations.
...
\quad We next consider a time-dependent perturbation of a single connection weight. The derivation parallels the node-perturbation case, so we state the corresponding formulas for the same four-node benchmark and then compare them with direct simulations. The relaxation response of the mean position is
\begin{equation}
...
\begin{figure}[H]
    \centering
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_xunddW.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_xdirdW.eps}}\\
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_funddW.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_fdirdW.eps}}\\
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_KounddW.eps}}
    \subfigure[]{\includegraphics*[width=.34\columnwidth]{Figure/Time_dependent/nonsteady_response_KodirdW.eps}}
    \caption{Comparison between Langevin simulations (dashed) and the time-dependent linear-response prediction (solid) for an edge perturbation in the four-node networks of Fig. \ref{graphN4}. The left column corresponds to the undirected equilibrium network with $\delta W_{12}(t)=\delta W_{21}(t)$, and the right column to the directed nonequilibrium perturbation $\delta W_{12}(t)$. Panels (a) and (b) show the mean-state responses $\langle x_1(t)\rangle-\langle x_1(0)\rangle$ and $\langle x_2(t)\rangle-\langle x_2(0)\rangle$. Panels (c) and (d) show the mean-force responses $\langle F_1(t)\rangle$ and $\langle F_2(t)\rangle$. Panels (e) and (f) show the equal-time covariance responses for $K_{11}(t,t)$ and $K_{12}(t,t)$. The close agreement between simulation and prediction demonstrates that the time-dependent response theory captures the propagation of coupling perturbations in both equilibrium and nonequilibrium settings.}
    \label{Time-dependent_dW}
\end{figure}
