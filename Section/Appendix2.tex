...
\end{eqnarray}
$\boldsymbol{{\mathscr F}} \equiv -\nabla_{\mathbf{W}} \Phi$ is the conjugate force when a connection weight $W_{\alpha\beta}$ is changed, and it can be calculated from (\ref{Falp}) to give
\begin{eqnarray}
...
\quad For time-dependent perturbations, there is the time-lagged correlation function between the observable and the conjugate force. To calculate the time-dependent change in the fixed point, similar to (\ref{scriptF}), we have the $\langle \delta \vec{x}(\tau) \vec{\mathscr{F}}^\intercal(0) \rangle$. Here $\tau$ denotes the time lag appearing in the correlation functions, while $t$ is used as the time variable in the response functions. For the linearized dynamics,
\begin{equation}
