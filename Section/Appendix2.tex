\section*{Appendix B: Detailed derivation of the conjugate force and the response function of some observables}\label{sec:AppendixA}
...
\quad For time-dependent perturbations, there is the time-lagged correlation function between the observable and the conjugate force. Similar to (\ref{scriptF_parameter}), we have the $\langle \delta \vec{x}(\tau) \vec{\mathscr{F}}^\intercal(0) \rangle$. Here $\tau$ denotes the time lag appearing in the correlation functions, while $t$ is used as the time variable in the response functions.
\begin{equation}
...
\end{equation}
\begin{eqnarray}
\langle \delta x_i(\tau) \delta x_j(\tau){\mathscr F}_\alpha(0)\rangle&=&  \Biggl\{- {1\over 2} \sum_{m,\ell} \sum_{\mu,\nu} \frac{\partial ({\bf K}_0^{-1})_{m\ell}} {\partial Q_{\mu\nu}}\frac {\partial Q_{\mu\nu}}{\partial p_\alpha}\\
...
\end{eqnarray}
is used in the last equality. Note that ${\bf K}_\tau= e^{\tau{\bf Q}}{\bf K}_0 $ and the RHS of (\ref{dxdxtauscrF}) can be theoretically calculated in terms of the network model parameters.
...
+{1\over 2} \Big[\partial_1^2h(X_i,X_j) \langle \delta x_i^2(t) {\mathscr F}_\alpha(0) \rangle\nonumber\\
...
\end{equation}
The non-steady time-dependent response function  of the fluctuation correlation  can be readily calculated to be
\begin{equation}
