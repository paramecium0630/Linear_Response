...
In the linearized noisy network dynamics, the steady-state probability distribution is approximated by a Gaussian distribution near its mean $\langle \vec{x} \rangle$, with $\Phi = \frac{1}{2} \delta \vec{x}^\intercal \mathbf{K}^{-1}_0 \delta \vec{x} = \frac{1}{2} (\vec{x}-\langle \vec{x} \rangle)^\intercal \mathbf{K}^{-1}_0 (\vec{x}-\langle \vec{x} \rangle)$. Note that $\langle {\vec x} \rangle$ depends on the parameters ${\vec p}$ through $\psi_{ss}$, and this dependence is generally difficult to compute explicitly in high-dimensional nonlinear Langevin systems. In the fixed-point approximation used throughout this work, we therefore replace $\langle \vec{x} \rangle$ by the noise-free fixed point $\vec{X}(\vec p)$. Throughout the paper, the control set $\vec p$ may include both deterministic parameters and noise strengths. Accordingly, one may regard ${\bf K}_0^{-1}$ more generally as ${\bf K}_0^{-1}({\bf Q}({\vec p},{\vec X}({\vec p})),\boldsymbol{\sigma}(\vec p))$. The conjugate force ${\vec{\mathscr F}}$ and correlations such as $\langle\delta \mathbf{B} \delta\vec{\mathscr F}\rangle_{ss,{\vec p}}$ can then be computed explicitly from (\ref{scriptF}) using this parameter dependence. For perturbations that act through the deterministic dynamics while leaving $\boldsymbol{\sigma}$ fixed, the dependence enters through $\mathbf{Q}$ and $\vec X$, and the conjugate force can be written as
\begin{eqnarray}
...
\end{eqnarray}
If $p_\alpha$ is instead a noise-strength parameter, then $\partial \mathbf{Q}/\partial p_\alpha=0$ and $\partial \vec{X}/\partial p_\alpha=0$, so the response arises from the explicit dependence on $\boldsymbol{\sigma}$; this case is treated separately in Subsec. B.4. One can derive, or directly verify from (\ref{Falp}), that
\begin{equation}
...
\end{eqnarray}
\subsubsection*{Explicit response function for the covariance matrix}
\begin{eqnarray}
    ...
    &+& \sum_\nu W_{s\nu} \partial_1\partial_2 h(X_s, X_\nu) \left( \frac{\partial \mathbf{K}_0}{\partial Q_{s\nu}} \right)_{ij} + \sum_\mu W_{\mu s} \partial^2_2 h(X_\mu, X_s) \left( \frac{\partial \mathbf{K}_0}{\partial Q_{\mu s}} \right)_{ij} \Bigg]. \label{chiKodr_ss_appB}
\end{eqnarray}
...
\begin{eqnarray}
    ...
    &+& \sum_\mu W_{\mu s} \partial_2^2 h(X_\mu, X_s) \left( \frac{\partial \mathbf{K}_0}{\partial Q_{\mu s}} \right)_{ij} \Bigg]. \label{chiKodW_ss_appB}
\end{eqnarray}
...
and therefore
\begin{equation}
    \chi_{ij(\alpha\alpha)}^{(ss)} = \left( \frac{\partial \mathbf{K}_0}{\partial \sigma_{\alpha\alpha}} \right)_{ij}.
    \label{chiKosigma_ss_appB}
\end{equation}
...
\end{eqnarray}
The time-dependent response function of the force is
\begin{equation}
...
\end{equation}
The time-dependent response function of the fluctuation correlation can then be written as
\begin{equation}
