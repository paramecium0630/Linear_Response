...
\quad We write down the explicit response formulas corresponding to the steady-state linear response theory developed in the previous section. We focus on the response of equal-time covariance matrix $\mathbf{K}_0$, and the observable becomes $\delta x_i \delta x_j$. We also present the corresponding response functions for the different types of parameter perturbations. From the general formula (\ref{response_function2}), the response function gives
\begin{equation}
...
\end{equation}
Since $\delta x_i$ does not explicitly depend on the parameters ($r_\alpha$, $W_{\alpha\beta}$, and $\sigma_{\alpha\alpha}$), the first term vanishes. The second term can be written as
\begin{eqnarray}
    \langle \delta (\delta x_i \delta x_j) \delta \mathscr{F}_\alpha \rangle_{ss, \vec{p}} &=& \langle (\delta x_i \delta x_j) \mathscr{F}_\alpha \rangle_{ss, \vec{p}} - \langle \delta x_i \delta x_j \rangle_{ss, \vec{p}} \langle \mathscr{F}_\alpha \rangle_{ss, \vec{p}}\nonumber\\
...
\begin{equation}
    ...
    \label{chiKodr_ss}
\end{equation}
This form shows that the response function of the covariance matrix is directly controlled by the diagonal element $Q_{\alpha\alpha}=-r_\alpha-\sum_j W_{\alpha j}$, namely by the local restoring parameter and the total incoming coupling associated with node $\alpha$.
...
\end{equation}
The higher orders of the derivative can be obtained by the similar manner. For the response with respect to $\sigma_{\alpha\alpha}$, we have
\begin{equation}
...
\end{equation}
The above formulas are further simplified when the network satisfies the equilibrium condition $\mathbf{Q}\boldsymbol{\sigma}=\boldsymbol{\sigma}\mathbf{Q}^\intercal$. In that case, the covariance matrix satisfies $\mathbf{K}_0 = -\frac{1}{2} \mathbf{Q}^{-1} \boldsymbol{\sigma}$, and therefore
\begin{equation}
...
\begin{equation}
    ...
    \label{chiKodr_eq}
\end{equation}
and the response functions for the change in a connection weight and a noise strength
\begin{equation}
    \chi_{ij(\alpha\beta)} = -\frac{\sigma}{2} Q^{-1}_{i\alpha} Q^{-1}_{j\alpha}+\frac{\sigma}{2} Q^{-1}_{i\alpha} Q^{-1}_{j\beta},
    \label{chiKodW_eq}
\end{equation}
\begin{equation}
    \chi_{ij(\alpha\alpha)} = -\frac{\sigma}{2} Q^{-1}_{i\alpha}.
    \label{chiKodsigma_eq}
\end{equation}
