...
% \subsection{}
We now develop a general steady-state linear response framework for noisy network dynamics under parameter perturbations. We focus on systems that relax to a stable fixed point and fluctuate around it under Gaussian white noise. When the parameters are perturbed as $\vec{p} \rightarrow \vec{p} + \delta \vec{p}$, the steady-state average of an observable ${\bf B}(\vec{x}; \vec{p})$ responds to this change. The goal is to express the response of steady-state observables in terms of quantities measured in the unperturbed steady state. We first consider the dependence of the noise-free fixed point on parameter changes. In (\ref{deterministic_force}), the noise-free fixed point satisfies
\begin{equation}
    \vec{F}(\vec{X}; \vec{p}) = 0.
\end{equation}
...
\end{equation}
Writing $\psi_{ss}=Z^{-1}e^{-\Phi}$, and defining the conjugate force with respect to parameters
\begin{equation}
    \vec{\mathscr{F}} \equiv -\nabla_{\vec{p}} \Phi,
...
\end{equation}
This is the steady-state linear response formula used throughout this work \cite{softmatter}. This formula requires a differentiable steady-state distribution around a stable fixed point, but does not require equilibrium. In the following sections, we apply this general framework to parameter changes in the intrinsic node parameter $r_\alpha$, the connection weight $W_{\alpha\beta}$, and the noise strength $\sigma_{\alpha\alpha}$. The detailed derivation of the conjugate force and the related response identities is summarized in Appendix~B.
